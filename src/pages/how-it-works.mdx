...
no only replace the `#overlay` but also the `#list`! Surely that's
not we want when we are closing an overlay, right?

Turns out, there are cases where we actually want that. When linking to another
filter from an overlay, for example. Or when jumping multiple steps back or forward
in the history.

Fragment Plugin **keeps track of the URL for each fragment**. If a fragment already
contains the current visit's URL, it **will be ignored for that visit**. There are cases where
you can help the plugin a little by providing information about the fragment URL beforehand.
Read more about that in the [DOM API](#dom-api)
...
In some cases, there is no way for Fragment Plugin to know the URL of the
